...
**Monthly rhythm.** Bills accrue against a lease with a `dueDay` → statement generated → SMS sent → payment recorded. Kit watches the overdue tail. An org can also charge a late fee on unpaid rent after a grace period (Settings → Late fees). That fee is a separate bill, one per overdue rent bill, and is never texted.
...
Status moves one way: Start takes Open to In progress, and Resolve takes In progress to Resolved with an optional note the tenant sees. The In-progress detail keeps that note inline above Close and Resolve, and Resolve submits the status change and note together. There is no skipping a step, reopening, editing, or filing on a tenant's behalf. Owners, admins, and members can all Start and Resolve. When someone else moved a request first, the API answers 409 and the detail shows where the request stands now. Photos are private and load with the session cookie. The earlier plan's Waiting on Vendor and Closed statuses are dropped. Assignee, priority, vendor, SMS, and resolution time per property are not built.

**Payment submissions (built).** `/bills/submissions` is the staff review queue for payment proof submitted through Residence Care. Status tabs cover Pending review, Approved, Rejected, Cancelled, and All history, with Pending review as the default. Review shows the receipt at a readable width in a scrollable preview and provides Open full size for the complete proof. Owners and admins can approve the exact submitted amount or reject with an optional tenant-visible reason inline. The sidebar shows the Pending review count, hides zero, refreshes on navigation, and updates after review.

**Parking Overview (built).** `/parking` is a live admin surface. It filters by property, and optionally by one terminal within it, since a terminal is bound to a property. Three sections: vehicles in park with plate, vehicle type, entry time, elapsed stay, terminal and attendant, oldest first, with a plate search and stays past twelve hours marked; a read-only log of collected exits and voided stays; and the shift cash queue, showing what the API expected against what the attendant declared, the variance in either direction, and an admin Confirm.

Guards alone perform parking in and parking out, on handheld devices. The admin web does not call parking-in or parking-out, and the staff API exposes no route for either. An admin corrects a wrong guard entry only by voiding it: open stays only, mandatory reason, and the original record is preserved with the actor and time recorded beside it. A collected exit is never reversed from the web. Confirming a shift records the sign-off and adjusts neither figure; `expectedCash` was snapshotted when the guard declared, so a rate edited later cannot restate it. Voiding and confirming are owner and admin actions. Cash only in this phase: no GCash and no waive on the Overview.

Revenue charts and a parking report catalog are not built. Settings manage vehicle types, flexible rate plans (duration tiers and overnight windows), parking rules, terminals, and parking attendants. Dashboard and Reports remain monitoring, read, and export surfaces without fake data.
