...
import { PAYMENT_METHOD_LABELS, PAYMENT_SOURCE_LABELS, PaymentMethod } from '../../../../core/models/enums';
...
import type { PaymentCta, PaymentTarget } from './payment-target';
...
@Component({
  ...
  templateUrl: './payment-workspace.html',
...
export class PaymentWorkspace {
  ...
  readonly dirty = this.errors.dirty;
  ...
  private readonly snapshot = signal<BillListItem | BillDetail | null>(null);
  private primedBillId: string | null = null;
  ...
  readonly methodOptions = computed(() =>
    staffCollectionMethodOptions(this.destinations(), this.snapshot()?.lease.unit.property.id),
  );
  ...
  private onTarget(target: PaymentTarget): void {
    if (this.primedBillId !== target.billId) {
      this.primeForm(target);
      return;
    }
  ...
  private primeForm(target: PaymentTarget): void {
    this.primedBillId = target.billId;
    this.errors.reset();
    ...
    this.payments.set([]);
    this.snapshot.set(target.seed ?? null);
    this.paidAmount.set(target.seed?.paidAmount ?? '0');
    this.balance.set(target.seed?.balance ?? '0');
    this.billedAmount.set(target.seed?.amount ?? '0');
    this.form.reset({
      amount: finiteAmount(target.seed?.balance ?? null),
      paidOn: new Date(),
      method: 'CASH',
      referenceNo: '',
      notes: '',
    ...
    this.loadDetail(target.billId);
  }

  private applySeedHint(seed: BillListItem | BillDetail | undefined): void {
    if (!seed) return;
    this.snapshot.set(seed);
  ...
  onSubmit(): void {
    ...
    const billId = this.target().billId;
    const { amount, paidOn, method, referenceNo, notes } = this.form.getRawValue();
    const memo = notes.trim();
    if (amount === null || paidOn === null || collectionMemoError(memo)) return;
    ...
    this.bills
      ...
      .subscribe({
        next: () => {
          ...
          this.changed.emit();
          this.loadDetail(this.target().billId);
        },
        error: (error: unknown) => {
          this.voidingId.set(null);
          this.toast.add({
