...
@Component({
  ...
  templateUrl: './vehicle-type-settings.html',
...
export class VehicleTypeSettings {
  ...
  readonly items = signal<VehicleTypeResponse[] | null>(null);
  ...
  load(): void {
    ...
    this.vehicleTypes
      ...
      .subscribe({
        ...
        error: (error: unknown) => this.error.set(apiErrorMessage(error, 'Could not load vehicle types.')),
      });
  }

  usageLabel(item: VehicleTypeResponse): string {
    if (item.ratePlanCount === 0) return 'Not used';
    return item.ratePlanCount === 1 ? '1 rate plan' : `${item.ratePlanCount} rate plans`;
  }
  ...
  startRename(item: VehicleTypeResponse): void {
  ...
  confirmArchive(item: VehicleTypeResponse): void {
    if (item.ratePlanCount === 0) {
      this.archive(item);
      return;
    }
    this.confirmation.confirm({
      header: 'Archive vehicle type',
      message: `Archive “${item.name}”? Rate plans keep any prices for it. It just leaves the picker for new vehicles.`,
  ...
  restore(item: VehicleTypeResponse): void {
  ...
  private archive(item: VehicleTypeResponse): void {
    this.busyId.set(item.id);
    this.vehicleTypes
      .archive(item.id)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (updated) => {
          this.busyId.set(null);
          this.replaceItem(updated);
          this.toast.add({ severity: 'success', summary: 'Vehicle type archived' });
        },
        error: (error: unknown) => {
          this.busyId.set(null);
          this.toast.add({
